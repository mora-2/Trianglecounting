...
class Server:
    ...
    def compute(self, NS):
        ...
        #             print("edge[", node_map[k[i]], "]", "[", node_map[k[j]], "]:", self.subgraph.get_edge_data(
        #                 node_map[k[i]], node_map[k[j]]), end="\t")
        #     print()
...
# if hasattr(nx.algorithms.triads, 'triad_count'):
#     print("triad_count function is available")
# else:
#     print("triad_count function is not available")
...
#     # 计算三元组数量
#     A = adj_mat @ adj_mat @ adj_mat
#     B = 3 * adj_mat @ adj_mat @ deg_mat
#     C = np.trace(adj_mat @ deg_mat @ adj_mat @ deg_mat)
#     D = np.trace(adj_mat @ deg_mat @ deg_mat @ deg_mat)
#     triads = np.array([A[i][i] - B[i][i] + C - D for i in range(len(adj_mat))])

#     # 将三元组数量存储为字典
#     triads_dict = {'003': triads[0], '012': triads[1], '102': triads[2],
#                    '021D': triads[3], '021U': triads[4], '111D': triads[5]}
