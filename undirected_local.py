...
class Server:
    def __init__(self):
        ...
        self.party_subgraph = nx.Graph()
        self.node_subgraph = nx.Graph()
    ...
    def generate_graph(self, G, node_list):
        self.party_subgraph.add_edges_from(G.edges(node_list))

    def compute(self, NODE, NS):
        if NODE in self.party_subgraph.nodes:
            self.triangle = nx.triangles(self.party_subgraph, NODE)
            self.node_subgraph.add_edges_from(self.party_subgraph.edges(NODE))
            # 建立节点映射表
            nodes = list(self.party_subgraph.nodes())
            node_map = {i: nodes[i] for i in range(len(nodes))}
            # # 打印节点映射表
            # print(node_map)

            wedges = triads_by_type(self.party_subgraph)['201']
            # print(wedges)
            ...
            for wedge in wedges:
                ...
                if self.party_label == list(set(self.party_label)):
                    self.W.update(
                        str(tuple(sorted(wedge, reverse=True))).encode('utf8'))
                    self.wedge += 1
        else:
            pass
...
def main():
   ...
   # 创建 ArgumentParser 对象
    ...
    for i in range(m):
        ...
        locals()[f'Party{i + 1}'].compute(NODE, list(G.graph['partition']))
    ...
    if NODE in G.nodes:
        ...
        print("node does not exit in the coupled graph")
